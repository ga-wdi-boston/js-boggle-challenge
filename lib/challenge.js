...
const bigBoggle = function bigBoggle(tray, pairs) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    // multiply first index value by 4 (simulating 5x5 grid)
    // then add the second value, that will be our index
    // then find the value of that index from the tray string.
...
const superBigBoggle = function superBigBoggle(tray, pairs) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    // multiply first index value by 4 (simulating 6x6 grid)
    // then add the second value, that will be our index
    // then find the value of that index from the tray string.
...
const infiniteBoggle = function infiniteBoggle(tray, pairs) {

  let gridSize = Math.sqrt(tray.length);
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    // then add the second value, that will be our index
    // then find the value of that index from the tray string.
    let stringIndex = (currentCoordinates[0] * gridSize) + currentCoordinates[1];
    res.push(tray[stringIndex]);
  }
